...
that turns the declarative case table in `gold_expectation_cases.py` into parametrized test items --
...
a case proves -- are wired up in a later unit of work. Until then, `pytest_generate_tests` here is
inert in real collection: no function in this module yet requests the fixture names it
parametrizes, and the published case table (`GOLD_CASES`) is empty. The builder itself is
exercised directly by this module's own tests, against throwaway cases, so its behavior does not
...
import sys
from typing import TYPE_CHECKING, Dict, FrozenSet, List, Optional, Sequence, cast
...
# --------------------------------------------------------------------------------------------
#
# GOLD_CASES is empty today (populated as later work fills the case table), and no real data
# source declares SupportTier.GOLD yet either. Both facts are exercised directly below rather
# than worked around: an empty table and an unclaimed tier are exactly the states the builder
# has to survive. Every other behavior -- exclusion honored, engine restriction applied, engine-
# less data source dropped, measurement substitution -- is proven against throwaway cases and
