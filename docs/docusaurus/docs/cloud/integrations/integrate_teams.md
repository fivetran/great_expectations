...
With GX Cloud's Microsoft Teams integration and alerts, you can notify standard channels in public Teams about Expectation failures Connect your GX Cloud workspace to Microsoft Teams to enable the following alert configuration options:
...
- The integration will connect to all of the public Teams in the Microsoft Teams tenant that the user who authorizes the connection has access to.
...
Your integration may **Error** if, for example, the user who configured it has their permissions within Microsoft Teams changed. If this happens, notifications might not be sent to Teams channels, but alert configurations will be kept intact so that Teams notifications will resume when the integration is reconnected. To reconnect the integration, do the following.
