...
The tutorial provides a functioning deployment of GX, Airflow, JupyterLab, and Postgres that can be run locally using Docker compose. Educational content is provided as JupyterLab notebooks that can be interactively run and modified to learn and explore how GX integrates with a data pipeline. Airflow is the orchestrator used for the tutorial environment, however, the examples can be adapted to any orchestrator that supports Python code.
